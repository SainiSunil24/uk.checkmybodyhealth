...
if ( typeof ProductForm !== 'function' ) {
	...
	class ProductForm extends HTMLElement {
		...
		onSubmitHandler(e) {
			...
			// START: Added code for auto-adding product
			const formData = new FormData(this.form);
			const mainProductId = formData.get('id');
			const mainProductQuantity = formData.get('quantity');
			
			// Get the selected selling plan, if any
			const selectedSellingPlan = formData.get('selling_plan');

			// Auto-add product details
			const autoAddProductId = '49243575419169';
			const autoAddSellingPlan = '689963499809';
			const autoAddQuantity = 1;

			// Check cart contents before adding
			fetch('/cart.js')
				.then(response => response.json())
				.then(cart => {
				// Check if auto-add item is already in cart
				let autoAddItemInCart = cart.items.find(item => item.id.toString() === autoAddProductId);
				let itemsToAdd = [
					{
					id: mainProductId,
				...
				// Add selling plan to main product if selected
				if (selectedSellingPlan) {
					itemsToAdd[0].selling_plan = selectedSellingPlan;
				}
				...
				if (!autoAddItemInCart) {
					itemsToAdd.push({
					id: autoAddProductId,
					selling_plan: autoAddSellingPlan,
					quantity: autoAddQuantity
					});
				}
			// END: Added code for auto-adding product

				// Modified to use itemsToAdd instead of this._serializeForm(this.form)
